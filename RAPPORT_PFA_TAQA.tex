...
\definecolor{ensiasdBar}{RGB}{68, 114, 196}
\definecolor{ensiasdLine}{RGB}{0, 112, 192}
\definecolor{ensiasdBlue}{RGB}{0, 102, 178}
...
%% ─── PAGE DE GARDE (OFFICIELLE ENSIASD / TAQA MOROCCO) ──────
\begin{titlepage}
\newgeometry{top=1.2cm, bottom=1.5cm, left=2.2cm, right=2.2cm}
\begin{center}

  %% En-tête institutionnel ENSIASD & Université Ibn Zohr
  \includegraphics[width=0.90\linewidth,keepaspectratio]{ensiasd_header}

  \vspace{0.25cm}
  %% Barre bleue institutionnelle
  \noindent\textcolor{ensiasdBar}{\rule{\linewidth}{7pt}}

  \vspace{0.65cm}
  %% Logo officiel TAQA Morocco (version fond clair)
  \includegraphics[width=5.6cm,keepaspectratio]{logo_taqa_dark}

  \vspace{0.5cm}
  {\LARGE\rmfamily RAPPORT DE STAGE}\\[0.35cm]
  {\large\rmfamily Fili\`ere : Sciences des Donn\'ees, Big Data \& IA}

  \vspace{0.65cm}
  %% Cadre et Titre du projet
  \noindent\textcolor{ensiasdLine}{\rule{\linewidth}{2pt}}
  \vspace{0.3cm}

  {\Large\bfseries\color{ensiasdLine}
   Syst\`eme Intelligent de Classification des Tickets IT\\[6pt]
   et Chatbot de R\'esolution Bas\'e sur une Architecture RAG}

  \vspace{0.25cm}
  \noindent\textcolor{ensiasdLine}{\rule{\linewidth}{2pt}}

  \vspace{0.6cm}
  {\normalsize\rmfamily Pr\'esent\'e par}\\[0.25cm]
  {\huge\bfseries\rmfamily Regragui Mohammed}

  \vspace{0.75cm}
  {\normalsize\rmfamily Soutenue le \hbox to 7.5cm{\dotfill} \`a l'Heure}

  \vspace{0.65cm}
  \noindent
  \begin{minipage}{\linewidth}
    {\normalsize\rmfamily Devant le jury}\\[0.3cm]
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{@{}p{2.0cm} p{9.8cm} p{3.5cm}@{}}
      \rmfamily Pr & \rmfamily Professeur \`a l'ENSIASD de Taroudant & \rmfamily Pr\'esident \\
      \rmfamily Pr & \rmfamily Professeur \`a l'ENSIASD de Taroudant & \rmfamily Examinateur \\
      \rmfamily Pr & \rmfamily Professeur \`a l'ENSIASD de Taroudant & \rmfamily Examinateur \\
      \rmfamily Pr & \rmfamily Professeur \`a l'ENSIASD de Taroudant & \rmfamily Encadrant \\
      \rmfamily Pr/Mr & \rmfamily Etablissement d'accueil : TAQA Morocco & \rmfamily Encadrant \\
    \end{tabular}
  \end{minipage}

  \vfill
  {\normalsize\rmfamily Ann\'ee Universitaire 2025- 2026}
...
L'interface de la plateforme \textbf{TAQA IT Smart Helpdesk} a ete concue selon les principes modernes de l'ergonomie logicielle et du design system \textit{Glassmorphism}, en parfaite harmonie avec la charte graphique officielle de TAQA Morocco (bleu nuit profond, cyan energetique et touches or). L'application s'articule autour de quatre interfaces principales concues en React~18.

%% ====================================================================
%% CAPTURE D'ÉCRAN 1 : PAGE D'AUTHENTIFICATION
%% Fichier image : report_figures/fig_ui_login.png
%% Description    : Écran de login avec logo TAQA et sélection du profil
%% ====================================================================
\subsection{Authentification et controle d'acces -- \texttt{LoginPage.jsx}}

L'ecran d'authentification constitue le point d'entree securise du systeme. Il integre une carte en verre depoli (\textit{glassmorphism}) positionnee sur un fond degrade aux couleurs institutionnelles de TAQA. 

L'utilisateur peut :
\begin{itemize}
  \item Saisir son adresse de messagerie corporative (\texttt{@taqa.ma}) et son mot de passe.
  \item Basculer de maniere intuitive entre le profil \textbf{Employe} (consommateur du service) et le profil \textbf{Technicien Support} (gestionnaire des incidents).
  \item Choisir dynamiquement son equipe technique de rattachement (\textit{Desktop Support}, \textit{Network Team}, \textit{Security Team} ou \textit{Application Support}) en mode technicien.
\end{itemize}

Comme illustre par la Figure~\ref{fig:ui-login}, cette interface garantit un aiguillage immediat vers le tableau de bord adapte des la validation des identifiants.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth,keepaspectratio]{fig_ui_login}
  \caption{Page d'authentification -- Interface en verre depoli et selection du profil utilisateur (\texttt{LoginPage.jsx})}
  \label{fig:ui-login}
\end{figure}

%% ====================================================================
%% CAPTURE D'ÉCRAN 2 : PORTAIL EMPLOYÉ & PRÉDICTION IA
%% Fichier image : report_figures/fig_ui_employee.png
%% Description    : Dashboard employé avec KPIs, tickets et formulaire IA
%% ====================================================================
\subsection{Portail Employe et saisie intelligente -- \texttt{TicketForm.jsx}}

Le portail employe regroupe l'ensemble des services dedies aux collaborateurs de la centrale thermique. L'ecran est structure en trois zones ergonomiques :
\begin{enumerate}
  \item \textbf{Bandeau de KPIs personnels} : visualisation en temps reel du nombre de demandes actives, des tickets resolus et du taux de disponibilite des systemes IT (99.8\,\%).
  \item \textbf{Fiche d'identite du collaborateur} : rappel de la direction, du poste d'affectation et des autorisations d'acces.
  \item \textbf{Formulaire de declaration avec prediction IA} : champ de saisie libre ou l'algorithme Machine Learning analyse le texte en temps reel. Des lors que l'employe tape sa requete, une requete asynchrone (\textit{debounce} 500\,ms) interroge l'endpoint \texttt{/api/predict}. Les badges interactifs affichent instantanement la categorie predite (ex: \textbf{Hardware}) et l'equipe assignee (ex: \textbf{Desktop Support}) avant meme la soumission du formulaire.
  \item \textbf{Tableau d'historique} : suivi synthetique des tickets soumis avec leurs identifiants uniques et leurs statuts (\textit{In Progress}, \textit{Resolved}).
\end{enumerate}

La Figure~\ref{fig:ui-employee} presente la vue complete du portail employe lors de la soumission d'une requete d'assistance.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth,keepaspectratio]{fig_ui_employee}
  \caption{Portail Employe -- Tableau de bord de suivi et formulaire avec prediction IA en temps reel (\texttt{TicketForm.jsx})}
  \label{fig:ui-employee}
\end{figure}

%% ====================================================================
%% CAPTURE D'ÉCRAN 3 : WIDGET SARA (CHATBOT & ESCALADE)
%% Fichier image : report_figures/fig_ui_chat_sara.png
%% Description    : Fenêtre de chat montrant l'escalade d'un incident critique
%% ====================================================================
\subsection{Widget conversationnel et escalade -- \texttt{ChatWidget.jsx}}

L'agente conversationnelle Sara est accessible sous forme de bulle interactive flottante (\textit{floating widget}) presente en permanence sur l'ecran. Elle offre une double fonctionnalite :
\begin{itemize}
  \item \textbf{Puces d'amorcage rapide} : boutons d'action en un clic pour les pannes courantes (\textit{Password Reset}, \textit{VPN AnyConnect}, \textit{SOS Technicien}).
  \item \textbf{Resolution RAG guidee} : lorsque la question correspond a l'une des 162 fiches certifiees, Sara restitue une procedure pedagogique en Markdown (etapes chiffrees, commandes a executer, liens intranet).
  \item \textbf{Escalade automatique} : lorsqu'un incident critique ou inconnu est signale (par exemple une alerte \textit{ransomware} ou panne majeure), l'algorithme detecte le seuil critique, alerte l'equipe specialisee et genere instantanement une carte visuelle de ticket (ex: \textbf{TCK-1024}, priorite \textit{CRITICAL}) avec transmission prioritaire.
\end{itemize}

La Figure~\ref{fig:ui-chat-sara} illustre la detection d'un ransomware par Sara et la creation immediate d'un ticket prioritaire pour l'equipe Securite.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth,keepaspectratio]{fig_ui_chat_sara}
  \caption{Widget Sara -- Prise en charge d'un incident de securite critique et auto-escalade avec ticket TCK-1024 (\texttt{ChatWidget.jsx})}
  \label{fig:ui-chat-sara}
\end{figure}

%% ====================================================================
%% CAPTURE D'ÉCRAN 4 : TABLEAU DE BORD TECHNICIEN SUPPORT
%% Fichier image : report_figures/fig_ui_support.png
%% Description    : Vue superviseur avec file d'attente, priorités et actions
%% ====================================================================
\subsection{Tableau de bord Superviseur et Technicien -- \texttt{SupportDashboard.jsx}}

L'interface dediee aux techniciens du support technique constitue la tour de controle operationnelle de l'ITSM. Elle offre :
\begin{itemize}
  \item \textbf{Indicateurs de charge globale} : nombre total de tickets recus, incidents en cours d'investigation, tickets resolus et alertes prioritaires necessitant une intervention immediate.
  \item \textbf{File d'attente filtrable par equipe} : menu deroulant permettant d'isoler en un clic les incidents alloues a une equipe specifique (\textit{Desktop Support}, \textit{Network Team}, \textit{Security Team}, \textit{Application Support \& SAP}).
  \item \textbf{Code couleur des priorites} : pastilles visuelles normalisees permettant de reperer d'un coup d'oeil le degre de gravite (\textcolor{red}{\textbf{Critical}}, \textcolor{orange}{\textbf{High}}, \textcolor{taqaGray}{\textbf{Normal}}).
  \item \textbf{Actions de resolution en un clic} : bouton \texttt{Mark Resolved} pour clore une intervention sans delai, et fonction \texttt{Reopen} pour reouvrir un dossier en cas de recidive.
\end{itemize}

La Figure~\ref{fig:ui-support} presente la file d'attente de l'equipe \textit{Desktop Support} avec la gestion des priorites et le traitement des requetes.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth,keepaspectratio]{fig_ui_support}
  \caption{Tableau de bord Superviseur -- File d'attente operationnelle de l'equipe Desktop Support (\texttt{SupportDashboard.jsx})}
  \label{fig:ui-support}
\end{figure}
